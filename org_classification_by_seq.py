# code to run predict species id from upstream region DNA seqs
...
def main():
    ...
    out_dir = 'out_5org_cls_rw_5fold' #'pred_out'
    split_dir = 'data/5org_cls_splits'

    seq_col_name = 'upstream_region' # TODO: put in config
    target_col_name = 'id' # TODO: put in config
    ...
    for fold in cvs:
        train_df = pd.read_csv(f'{split_dir}/cv{fold}_train.tsv',sep='\t')
        test_df = pd.read_csv(f'{split_dir}/cv{fold}_test.tsv',sep='\t')
